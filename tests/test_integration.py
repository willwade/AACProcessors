...
def test_coughdrop_extract_real_obz(demo_obz_file):
    ...
    expected_texts = [
        "yes", "no", "more", "stop",      # Common core words
        "i", "you", "it", "want",         # Pronouns and common verbs
        "like", "help", "go", "look"      # Action words
    ]
    for text in expected_texts:
        assert any(text.lower() == t.lower() for t in texts), \
...
def test_coughdrop_extract_real_obf(demo_obf_file):
    ...
    for verb in expected_verbs:
        assert any(verb.lower() == t.lower() for t in texts), \
...
def test_snap_extract_real(demo_snap_file):
    ...
    assert len(texts) >= 260, \
        f"Expected at least 260 total texts, found {len(texts)}"
    ...
    with tempfile.NamedTemporaryFile(suffix=".obz") as output:
        ...
        for original in translations:
            if original != "target_lang":
                ...
                assert any(expected == t for t in translated_texts), \
...
def test_gridset_translate_real(demo_gridset_file):
    ...
    translations = {
        text: f"TEST_{text}" 
        for text in list(texts)[:5] if text.strip()
    }
    translations["target_lang"] = "test"
