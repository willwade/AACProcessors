...
def test_file_path_handling():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        output_path = os.path.join(
            temp_dir, f"{os.path.splitext(os.path.basename(test_file))[0]}_es.test"
        )
...
def test_archive_handling():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            processor.file_path == test_file
        ), "Archive path not set correctly after extraction"
        assert (
            processor.original_file_path == test_file
        ), "Original archive path not set correctly"
        ...
        assert os.path.exists(
            output_path
        ), f"Output archive not created at {output_path}"
        assert (
            processor.file_path == test_file
        ), "Archive path changed during translation"
        assert (
            processor.original_file_path == test_file
        ), "Original archive path changed"
...
def test_temp_directory_cleanup():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            len(processor._temp_dirs) == 0
        ), "Temp dirs not cleaned up after extraction"
        ...
        assert (
            len(processor._temp_dirs) == 0
        ), "Temp dirs not cleaned up after translation"
